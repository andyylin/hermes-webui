...
async function newSession(flash, options={}){
  ...
  _newSessionInFlight=(async()=>{
    ...
    _oldestIdx=0;
    clearLiveToolCards();
    // Explicit profile switch wins. A selected project's default wins for
    // project quick-create; otherwise inherit the current conversation, then profile default.
    ...
    const hasProjectOption=Object.prototype.hasOwnProperty.call(options,'project_id');
    const sessionWs=(!switchWs&&!projectWs&&!hasProjectOption&&S.session)?S.session.workspace:null;
